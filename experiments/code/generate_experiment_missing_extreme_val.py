...
def main():
    ...
    y_train_nan = add_nan_extreme_values(y_train, percentage=0.1)
    y_train_nan_tensor = torch.tensor(y_train_nan, dtype=dtype)

    y_stop_nan = add_nan_extreme_values(y_stop, percentage=0.1)
    y_stop_nan_tensor = torch.tensor(y_stop_nan, dtype=dtype)

    y_calibration_nan = add_nan_extreme_values(y_calibration, percentage=0.1)
    y_calibration_nan_tensor = torch.tensor(y_calibration_nan, dtype=dtype)

    y_test_nan = add_nan_extreme_values(y_test, percentage=0.1)
    ...
    for alpha in tab_alpha:
        ...
        file_path = Path("../results/results_missing_extreme.csv")
